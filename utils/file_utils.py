...
def build_dgl_graph(u, v, dis, node_type: np.array, node_size: np.array, node_pos: np.array):
    g = dgl.graph((u, v))
    ...
    g.ndata['coord'] = torch.FloatTensor(node_pos)
    return g
...
def load_pp_file(file_path: Path):
    ...
    for i in range(n_nodes):
        for j in range(i + 1, n_nodes):
            ...
            dis.append(pos2edis(node_pos[i], node_pos[j]))   # use Euclidean distance

    u, v = u + v, v + u
    dis = dis + dis

    g = build_dgl_graph(u, v, dis, node_type, node_size, node_pos)
